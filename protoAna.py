...
class PcapDecode:
    def __init__(self):
        # ETHER:读取以太网层协议配置文件
        with open('./protocol/ETHER.txt', 'r', encoding='UTF-8') as f:
            ethers = f.readlines()
        ...
        # IP:读取IP层协议配置文件
        with open('./protocol/IP.txt', 'r', encoding='UTF-8') as f:
            ips = f.readlines()
        self.IP_DICT = dict()
        for ip in ips:
            ip = ip.strip().strip('\n').strip('\r').strip('\r\n')
        ...
        # PORT:读取应用层协议端口配置文件
        with open('./protocol/PORT.txt', 'r', encoding='UTF-8') as f:
            ports = f.readlines()
        self.PORT_DICT = dict()
        for port in ports:
        ...
        # TCP:读取TCP层协议配置文件
        with open('./protocol/TCP.txt', 'r', encoding='UTF-8') as f:
            tcps = f.readlines()
        self.TCP_DICT = dict()
        for tcp in tcps:
            tcp = tcp.strip().strip('\n').strip('\r').strip('\r\n')
        ...
        # UDP:读取UDP层协议配置文件
        with open('./protocol/UDP.txt', 'r', encoding='UTF-8') as f:
            udps = f.readlines()
        self.UDP_DICT = dict()
        for udp in udps:
            udp = udp.strip().strip('\n').strip('\r').strip('\r\n')
